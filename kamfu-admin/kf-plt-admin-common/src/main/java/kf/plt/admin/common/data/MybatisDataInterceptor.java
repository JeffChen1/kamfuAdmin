...
package kf.plt.admin.common.data;
...
public class MybatisDataInterceptor implements Interceptor {
    private IUserGroupDataService userGroupDataService;

    public MybatisDataInterceptor(IUserGroupDataService userGroupDataService, String... ignoreMappers) {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (annotation != null) {
            ...
            // 拼接部门数据sql
            if (userGroupDataService != null) {
                List<String> userDataGroupIds = userGroupDataService.getUserDataGroupIds(BaseContextHandler.getUserID());
                if (userDataGroupIds != null && userDataGroupIds.size() > 0) {
                    for (int i = 0; i < userDataGroupIds.size(); i++) {
                        if (i == 0) {
                            whereSql.append(" or ");
                        }
                        whereSql.append(addAlias(plain, annotation.groupField())).append(" = '").append(userDataGroupIds.get(i)).append("' ");
                    }
                }
            }
            whereSql.append(" ) ");
